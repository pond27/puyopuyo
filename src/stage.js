    ...
    static checkErase(startFrame) {
        ...
        for (let y = 0; y < Config.stageRows; y++) {
            for (let x = 0; x < Config.stageCols; x++) {
                ...
                const puyoXY = this.board[y][x]
                const puyoColor = puyoXY && puyoXY.puyo;
                checkSequentialPuyo(x, y);
                if (sequencePuyoInfoList.length == 0 || sequencePuyoInfoList.length < Config.erasePuyoCount[puyoXY.puyo]) {
